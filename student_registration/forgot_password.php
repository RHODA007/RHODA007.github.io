...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($admin) {
        ...
        $stmt = $pdo->prepare("UPDATE admins SET reset_token = ? WHERE username = ?");
        $stmt->execute([$token, $username]);

        // Normally, you would email the token link, but here we'll show it directly
        $msg = "Reset link: <a href='reset_password.php?token=$token'>Click to reset password</a>";
    } else {
...
<div class="glass-card">
    ...
    <form method="POST">
        ...
        <button type="submit" class="btn btn-primary btn-custom w-100">Send Reset Link</button>
    </form>
