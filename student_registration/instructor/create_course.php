...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $description) {
        $stmt = $pdo->prepare("INSERT INTO courses (title, description, category, level, duration, instructor_id, created_at) 
                               VALUES (?, ?, ?, ?, ?, ?, NOW())");
        if ($stmt->execute([$title, $description, $category, $level, $duration, $instructor_id])) {
            $message = "<div class='alert alert-success'>Course created successfully!</div>";
        } else {
            $message = "<div class='alert alert-danger'>Failed to create course. Try again.</div>";
        }
    } else {
...
<div class="sidebar">
  ...
  <ul class="sidebar-menu">
    ...
    <li><a href="create_course.php" class="active"><i class="fas fa-plus-circle"></i> Create Course</a></li>
...
<div class="main-content">
  ...
  <div class="card">
    <form method="POST" action="">
      ...
      <div class="mb-3">
        <label class="form-label">Duration (e.g. 6 weeks)</label>
